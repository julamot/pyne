...
std::string pyne::Material::write_material(int id)
{
  ...
  // set fluka_name; fill metadata with same if necessary
  ...
  // could be empty
  std::cout << "pyne: ms.str(): " << ms.str() << "\tthat was it" << std::endl;
  return ms.str();
}
...
std::string pyne::Material::write_compound(int id)
{
  ...
  // This line can stay, but it should be removed elsewhere
  ss << material_line(znum, atomic_mass, id, compound_name);
  // ad deleted this; but the material card for a compound uses up an id
  // id++;
